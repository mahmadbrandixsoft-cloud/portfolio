...
document.addEventListener('click', (e) => {
  ...
  if (e.target.closest('#mobile-menu') || e.target.closest('#nav-toggle')) return;
  setMenu(false);
});

/* ====== Theme toggle ====== */
const THEME_KEY = 'ahmad-theme';
...
function initGrid(){
  ...
  function draw(time){
    ...
    const stroke = (cs.getPropertyValue('--grid-stroke').trim() || '120, 130, 180');
    const glow   = (cs.getPropertyValue('--grid-glow').trim()   || '99, 130, 246');
    const accent = (cs.getPropertyValue('--grid-accent').trim() || '168, 130, 246');
    ...
    for (let i=0;i<cols;i++){
      for (let j=0;j<rows;j++){
        ...
        ctx.strokeStyle = `rgba(${stroke}, ${0.10 + t * 0.35})`;
        ctx.lineWidth = 1;
        ctx.strokeRect(x + 0.5, y + 0.5, cellSize, cellSize);
        if (t > 0.05){
          // glow box near cursor
          ctx.fillStyle = `rgba(${glow}, ${t * 0.32})`;
          ctx.fillRect(x, y, cellSize, cellSize);
          // inner accent square
          if (t > 0.5){
            ctx.fillStyle = `rgba(${accent}, ${(t-0.5) * 0.5})`;
        ...
        if (diag < 0.4){
          ctx.fillStyle = `rgba(${accent}, ${0.04 * (1 - diag/0.4) * (1-wave*0.4)})`;
          ctx.fillRect(x, y, cellSize, cellSize);
        }
      }
    }
